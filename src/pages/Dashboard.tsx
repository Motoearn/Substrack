...
export function Dashboard() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      if (subscribersResult.data) {
        ...
        const cancelledSubscribers = allSubscribers.filter(
        ...
        const upcomingRenewals = activeSubscribers.filter(
          (s) =>
            s.next_renewal_date && new Date(s.next_renewal_date) <= nextWeek
        ).length
        ...
        const currentMonthRevenue =
          currentMonthTransactions.data?.reduce(
            (sum, t) => sum + t.amount,
            0
          ) || 0
        ...
        const totalRevenue =
          allTimeTransactions.data?.reduce((sum, t) => sum + t.amount, 0) || 0
        ...
        const lastMonthActiveSubscribers =
          allSubscribersForMRR.data?.filter((s) => {
            ...
            const isStartedBeforeThisMonth =
        ...
        const subscribersByMonth = prepareSubscriberChartData(
          allSubscribers,
          dateRange
        )
        ...
        if (plansResult.data) {
          const revenueByPlan = prepareRevenueByPlanData(
            allSubscribers,
            plansResult.data
          )
      ...
      if (allTransactions.data) {
        const revenueByMonth = prepareRevenueChartData(
  ...
  const prepareSubscriberChartData = (
    subscribers: any[],
    range: string
  ): SubscriberChartData[] => {
        ...
        <div className='bg-white p-3 rounded-lg shadow-lg border border-gray-200'>
          ...
          {type === 'revenue' ? (
            ...
            <>
              ...
              {payload[1] && (
                <p className='text-red-600'>Churned: {payload[1].value}</p>
              )}
  ...
  return (
    <DashboardLayout title='Dashboard'>
      ...
      <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6'>
        <div className='bg-white p-6 rounded-xl shadow-sm flex items-center justify-between'>
          <div>
            ...
            <div
              className={`text-xs flex items-center gap-1 mt-1 ${getGrowthColor(
                stats.monthlyRevenueGrowth
              )}`}
            >
          ...
          <div>
            ...
            <p className='text-2xl font-bold text-gray-800'>
              ...
              {stats.activeSubscribers > 0
                ? (stats.monthlyRevenue / stats.activeSubscribers).toFixed(2)
                : '0.00'}
            </p>
      ...
      {/* Row 3: Subscriber Growth Chart | Revenue Trend Chart */}
        ...
        <div className='bg-white p-6 rounded-xl shadow-sm'>
          ...
          <ResponsiveContainer width='100%' height={280}>
            <BarChart data={subscriberChartData}>
              ...
              <Tooltip
                content={(props) => (
                  <CustomTooltip {...props} type='subscribers' />
                )}
              />
              <Legend />
              <Bar
        ...
        <div className='bg-white p-6 rounded-xl shadow-sm'>
          ...
          <ResponsiveContainer width='100%' height={280}>
            <AreaChart data={revenueChartData}>
              ...
              />
              <Tooltip
                content={(props) => <CustomTooltip {...props} type='revenue' />}
              />
              <Area
                type='monotone'
                dataKey='revenue'
      ...
      {/* Row 4: Revenue by Plan (Pie Chart) | Recent Activity */}
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6 mt-6'>
        <div className='bg-white p-6 rounded-xl shadow-sm'>
